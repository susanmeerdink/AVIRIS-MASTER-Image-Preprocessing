...
foreach fl, fl_list do begin
  ...
  foreach date, date_list do begin
    ...
    aviris_list = file_search(output_path + fl + '\1 - Resized Imagery\*' + date + '*', /WINDOWS_SHORT_NAMES) ;Get list of aviris files for this flightline & date
    ...
    time_list = file_search(output_path + fl + '\2 - Time Files\*' + date + '_UTC_36', /WINDOWS_SHORT_NAMES) ;Get list of aviris files for this flightline & date
...
;      PRINT, AVIRIS_resize(e, input, spatial_dim, outputResample)
      ...
      outputMask = output_path + fl + '\4 - Mask File\' + file_basename(outputLayer) + '_mask'
      outputImg = output_path + fl + '\5 - Corrected Imagery\' + file_basename(outputLayer) + '_Cor'
